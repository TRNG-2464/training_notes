...
/*
    ...
    When using a for-of loop, you establish a variable that is used as a placeholder which 
    adopts the value of each element of a collection.

    Another way to look at this, imagine creating a variable for each item in the letterArray
    then printing that variable to the console...this is what the variable 'letter' represents. i.e.:
...
/*
    When iterating through objects, the For-in loop will work natively to iterate
    through all fields of a defined class/object. However, the For-Of loop requires
    the creation of an iterator or generator - here we create a generator
*/
class Person {
    ...
    // required for the for-of loop - notice that we are iterating back-wards for this example
    *[Symbol.iterator]() {  // Iterator's next method gives you an object with a value and a boolean
        yield this.age; // { this.age, false }
        yield this.last; // { this.last, false }
        yield this.first; // { this.first, true }
    }
}
