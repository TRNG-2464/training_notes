...
// The syntax you see below is creating an 'anonymous function'
// these anonymous functions can be passed to another function or assigned to a variable
// like below
...
/*
    In Javascript we can add properties "on-the-fly" (after the object has been created) 
    by accessing a property that does not exist currently on my object. Note, this does 
    not work in every programming language
*/
...
class Animal {
  constructor(name) {
    this.name = name; // Set up initial state when a new instance is created
  }

  speak() {
    // This method is added to Animal.prototype, not copied per instance
...
class Dog extends Animal {
  constructor(name, breed) {
    super(name); // MUST be called before using 'this' — runs Animal's constructor
    this.breed = breed;
  }

  speak() {
    // Overriding the parent method - we can still access the parent's implementation using 'super'
    console.log(`${this.name} barks! (Breed: ${this.breed})`);
  }

  oldSpeak() {
    super.speak(); // Calls Animal's version of speak() first - not required to override, just showing that we can
  }
}
...
// Remember that child classes must call super() in the child constructor
class BrokenCat extends Animal {
  constructor(name) {
    // Missing super(name) here
    this.name = name; // ReferenceError: Must call super constructor before accessing 'this'
  }
}

// const brokenCat = new BrokenCat("Whiskers"); // Would throw an error if run
// Confirming that classes are built on the same prototype system
